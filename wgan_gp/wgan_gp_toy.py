...
# imgs, y = datasets.make_moons(n_samples=1000, noise=0.1)
...
for epoch in range(opt.n_epochs):
    ...
    if epoch % opt.n_critic == 0:
        ...
        if epoch % opt.sample_interval == 0:
            ...
            val = torch.mean(real_validity) - torch.mean(fake_validity)
            r, g = np.ones((opt.batch_size,)) / opt.batch_size, np.ones((opt.batch_size,)) / opt.batch_size
            M = ot.dist(fake_imgs.cpu().detach().numpy(), real_imgs.cpu().detach().numpy(), metric='euclidean')
            wass = ot.emd2(r, g, M)
