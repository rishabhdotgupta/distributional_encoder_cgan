...
parser.add_argument("--reg", type=int, default=0, help="output dim of the encoder")
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        src_gen, dest_gen = gen_imgs[src].reshape(opt.num_paths, -1), generator(src_z, dest_enc).reshape(opt.num_paths, -1)
    ...
    ot1.append(ot_loss_total.detach().cpu().numpy())
    ...
    plt.figure()
    plt.subplot(211)
    plt.hist(real_imgs[0].cpu().numpy(), bins=50)
    plt.subplot(212)
    plt.hist(gen_imgs[0].detach().cpu().numpy(), bins=50)
    ...
    if epoch % 5 == 4:
        ...
        plt.xlabel("Epochs")
        plt.ylabel("Loss")
        plt.savefig("losses/test_%d.png" % epoch)
